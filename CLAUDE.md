...
Audio is normalized at upload: `POST /api/jobs` saves the raw upload (served verbatim for playback/export) and immediately transcodes it to `jobs/<id>/align.wav` (16 kHz mono s16 wav, the aligner's input format) — unparseable audio fails right at upload with `音频无法解析` 400 instead of dying mid-align; a retry of an old job (no `align.wav` yet) re-transcodes on demand via `_ensure_align_wav()`. Alignment outlives HTTP timeouts, so it's job-based: `POST /api/jobs` returns a job id immediately; poll `GET /api/jobs/<id>` for status; `GET /api/jobs/<id>/result` when done; audio at `GET /api/jobs/<id>/audio` (Range requests → 206, gives the player seek). One daemon worker thread pulls from a queue. Jobs persist under `jobs/<id>/` (`meta.json` written tmp-file + `os.replace`; in-memory dict is authoritative). On restart, queued/running jobs are marked failed — their worker is gone. Cancellation: `progress_cb` raises `JobCancelled` at a chunk boundary. Failed jobs can be re-queued via `POST /api/jobs/<id>/retry` — it reuses the on-disk `audio*` + `text.txt`, no re-upload; cover uploads ride the same `POST /api/jobs` (optional `cover` file, saved `jobs/<id>/cover.<ext>`). `seed_sample_job()` pre-populates a `sample` (done) job from `src/data/` + `src/align.json` on first start. Run with `use_reloader=False` — the reloader forks and loads the model twice.

Scroll export to MP4 follows the same job shape: `POST /api/jobs/<id>/export` → poll `GET /api/jobs/<id>/export` (in-memory status, progress % parsed from the subprocess stdout) → `GET /api/jobs/<id>/export.mp4` when done. The server shells out to `src/frontend/scripts/export-scroll.mjs` via `node` (and `ffmpeg` on PATH); the script needs `@napi-rs/canvas` **declared in package.json** (pnpm prunes undeclared packages on install — it once vanished and broke exports with `ERR_MODULE_NOT_FOUND`); state lives only in memory, so after a restart an in-flight export is gone and re-POST regenerates. `POST /api/jobs/<id>/export?force=1` re-renders a done export（重新导出; queued/rendering 时 force 无效；`ensure()` 查询与认领在锁内原子完成，并发 force 只开一个渲染线程，force 另有 60s 冷却（`ExportTooSoon` → 429「重新导出太频繁」），渲染全局最多 `BoundedSemaphore(2)` 个并发，其余排队显示「排队中」）; a done state carries `elapsed_sec` (this render's wall time, shown as 耗时 in the UI); the render page auto-starts the export when no export has run yet (job just finished / server restarted). The mp4 lands at `jobs/<id>/scroll.mp4` (gitignored). Rendering is **segment-parallel**: `ScrollRenderer.draw(ctx, t)` is pure in `t` (no frame-to-frame state), so the script splits frames into `min(8, 核数, 每段≥1200 帧)` segments rendered by same-script `--range` child processes (each rawvideo → own x264 pass `-crf 22 -preset slow`, final concat copies video + re-encodes audio at `-b:a 96k`), then concat + audio mux; the orchestrator keeps the `\rNN%` progress contract. The mp4 gets an **embedded cover**: the frame at `min(20s, 末帧)` is drawn by the renderer, turned into a 1s 定格 segment, and prepended as the concat's first item — previews/thumbnails read the first frame; audio is delayed 1s (`-itsoffset`), so total = audio + 1s (mp4 has no video-poster field, unlike audio album art). 22-min video ≈ 2 min wall (**~2.5×** vs sequential). Also **no initial fade in** — the former `wrapA` paper alpha ramp at 卷头 was removed from `scroll.ts`.

## Conventions

- Python docstrings/comments, error messages, and UI strings are in Chinese — match that.
- `align_core.py` is the import target — the only real aligner (legacy `main.py` and the scratch `asr.py` were removed).
- `frontend/` (React 19 + Vite + TS, React Compiler via Babel preset, oxlint) is the workbench UI: a step-oriented wizard (react-router v8 — `RouterProvider` comes from `react-router/dom`, everything else from `react-router`): `/` dashboard job list → `/create` upload audio+text → `/jobs/:id` align status (polls; when `done` shows the cue list with a time playhead) → `/jobs/:id/render` the 竹简卷轴 scroll view (+ video export). It's a guarded flow — RenderPage bounces to the align page unless the job is `done`. `JobsProvider` (context) owns health + job-list polling (2s while any job is active, else 15s); `useJob(id, pollMs)` handles per-job status polling; scroll/export polling lives in `ScrollPlayer`. Dev traffic goes through Vite's proxy (`/api` → `127.0.0.1:5000` in `vite.config.ts`), so the Flask app needs no CORS. It does not consume the static `align.json`. No test suite.
- 竹简卷轴 (`src/frontend/src/scroll.ts` + `ScrollPlayer.tsx`) renders the aligner timestamps as a bamboo scroll: vertical text (12 rows/col) reading right→left (the whole scene is drawn mirrored — story column 1 at the right end), the inking character pinned at a pen position at the left edge, already-written text sliding right one column per column, cycling when the audio loops. 首列之前（无已入墨字）纸卷停在「首列前一列」，起始帧露出卷书尾杆 + 封面条 + 首列空白，首字入墨与其他列头字一样从笔位左一列滑入。Punctuation keeps its aligned timestamp but is drawn in vertical presentation form (`toVertical()` — `。`→`︒` etc.; punctuation comes from `buildChars` walking the full `data.text` against the alnum-only word stream, not from the words themselves). An optional cover image — uploaded with the job, saved `jobs/<id>/cover.<ext>`, served at `/api/jobs/<id>/cover` — is drawn full-height as a strip at the paper's story-start end; `CoverSource` in `scroll.ts` is the shared adapter (browser `HTMLImageElement` vs `@napi-rs/canvas` `loadImage`), and the server appends `--cover` when exporting. `scroll.ts` is pure canvas and shared verbatim between the browser player and `scripts/export-scroll.mjs` (headless MP4 via @napi-rs/canvas → ffmpeg rawpipe; font in exports is `fc-match` on system Noto Serif CJK SC — the rotated FangSong variant must be avoided).
